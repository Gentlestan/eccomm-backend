...
class PaystackVerifyView(APIView):
    """
    Verify Paystack payment, validate stock, create payment record,
    deduct inventory, and store full shipping details.
    """
    ...
    def post(self, request):
        ...
        # Normalize pending_order_id safely
        try:
            pending_order_uuid = (
                pending_order_id
            ...
            return Response({"detail": "Invalid pending_order_id."}, status=status.HTTP_400_BAD_REQUEST)

        # Fetch pending order
        try:
            order = Order.objects.get(public_id=pending_order_uuid, user=request.user, status="pending")
